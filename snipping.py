...
class Application():
    def __init__(self) -> None:
        ...
        self.main_window = self.createWindow("Snipping Tool Max Speed")
        ...
        self.title = tk.Label(self.main_window, text="SNIPPING TOOL Max Speed", font=("HELVETICA",28,"bold"),bg=self.bg,fg=self.fg)
        self.title.grid(row=0,column=0,sticky=N,pady=(40,20),padx=40)
        self.desc_direc = tk.Label(self.main_window, text="hãy chọn đường dẫn!",font=("ARIAL",12),bg=self.bg,fg=self.fg)
        self.desc_direc.grid(row=1,column=0,padx=20, pady=20)
        self.snip_button = tk.Button(self.main_window,text="chọn đường dẫn", font=("TIMES NEW ROMAN",6),bg=self.fg,fg=self.bg, height = 1, width = 10, command = self.choseDire, bd=7, relief=RAISED)
        self.snip_button.grid(row=2,column=0, pady=30,padx=10)
        self.desc = tk.Label(self.main_window, text="ẤN NÚT BÊN DƯỚI ĐỂ CẮT VỊ TRÍ ẢNH.(Phím tắt ctrl-c)",font=("ARIAL",12),bg=self.bg,fg=self.fg)
        self.desc.grid(row=3,column=0,padx=20, pady=20)

        self.snip_button = tk.Button(self.main_window,text="cắt ảnh", font=("TIMES NEW ROMAN",14),bg=self.fg,fg=self.bg, height = 2, width = 12, command = self.snipImage, bd=7, relief=RAISED)
        self.snip_button.grid(row=4,column=0, pady=30,padx=20)
    ...
    def createWindow(self,title):
        ...
        window.title(title)
        window.geometry("")
        window.bind('<Control-c>',lambda event: self.snipImage())
        return window

    def takeBoundedScreenShot(self, x1, y1, x2, y2):
        
        date_time = datetime.now().strftime("%d-%m-%Y-%H-%M-%S")
        path = self.desc_direc.cget("text")+ "/Capture"+date_time+".png"
        im = pyautogui.screenshot(region=(x1, y1, x2, y2))
        im.save(path+".png")
    def choseDire(self):
        text = self.desc_direc.cget("text")
        textDirec = askdirectory()
        if textDirec != '':
            self.desc_direc.config(text =   textDirec ) 
        else :
            self.desc_direc.config(text =   text ) 
    def snipImage(self):
        text = self.desc_direc.cget("text")
        if  text != "hãy chọn đường dẫn!":
            self.rect = None
            self.x = self.y = 0
            self.start_x = None
            self.start_y = None
            ...
            messagebox.showinfo("Thất bại","Hãy chọn đường dẫn")

    def on_button_release(self, event):
        # separator = ttk.Separator(self.main_window, orient='horizontal') 
        # separator.place(y=40,relx=0, rely=0.47, relwidth=1)

        # self.inf = tk.Label(self.main_window,text="Thông tin", font=("HELVETICA",14,"bold"),bg=self.bg,fg=self.fg)
